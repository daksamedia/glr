...
class Product
{
    ...
    function readOne(){
        ...
        //read single record
        $query = "SELECT
                c.name AS category_name, p.id, p.name, p.cover, p.bio, p.location, p.location_data, p.ratings, p.reviews, p.price, p.category_id, p.created
            FROM
                " . $this->table_name . " p
                   LEFT JOIN
                    categories c ON p.category_id = c.id
    ...
    function readMy(){
        ...
        //read single record
        $query = "SELECT
                c.name AS category_name, p.id, p.user_id, p.name, p.cover, p.bio, p.location, p.location_data, p.ratings, p.reviews, p.price, p.category_id, p.created
            FROM
                " . $this->table_name . " p
                   LEFT JOIN
                    categories c ON p.category_id = c.id
	...
	function create(){
		...
		$query = "INSERT INTO
				  ". $this->table_name ."
				  SET
					name=:name, price=:price, user_id=:user_id, location=:location, bio=:bio, category_id=:category_id, location_data=:location_data, created=:created";
		...
		$this->location_data=htmlspecialchars(strip_tags($this->location_data));
	...
	function search_filter(){
        ...
        if (isset($this->province)) {
            $prov_array   = explode(',', $this->province);
            $prov_in      = str_repeat('?,', count($prov_array) - 1) . '?';
            $prov_q       = "AND JSON_VALUE(location_data, '$.province') IN ($prov_in)";
        } else {
            $prov_q       = "";
        }
        ...
        // explode city
        if (isset($this->city)) {
            $city_array   = explode(',', $this->city);
            $city_in      = str_repeat('?,', count($city_array) - 1) . '?';
            $city_q       = "AND JSON_VALUE(location_data, '$.city') IN ($city_in)";
        } else {
            $city_q       = "";
        }
        ...
        if (isset($this->category_id)) {
            $cat_array   = explode(',', $this->category_id);
            $cat_in        = str_repeat('?,', count($cat_array) - 1) . '?';
            $cat_q       = "AND category_id IN ($cat_in)";
        } else {
            $cat_q = "";
        }
        ...
        if (isset($this->order_by)) {
            $sort_q = "ORDER BY ". $this->order_by ." ". $this->order_type;
        } else {
            $sort_q = "ORDER BY created DESC";
        }

        // query
        $query = "SELECT * FROM ". $this->table_name ." WHERE name LIKE ? ". $prov_q ." ". $city_q ." ". $cat_q ." ". $sort_q;
        ...
        // if (isset($this->order_by)) {
        //     array_push($all_array, $this->order_by);
        // }

        // if (isset($this->order_type)) {
        //     array_push($all_array, $this->order_type);
        // }

        //execute
        $stmt->execute($all_array);

        //return
        return $stmt;
	}

    //older version search (still workaround)
    //advanced search products
	function search_filter_old($location, $category, $price, $rating){
		//select all query
        $query = "SELECT
                  c.name AS category_name, p.id, p.name, p.cover, p.location, p.ratings, p.reviews, p.price, p.category_id, p.created
                  FROM " . $this->table_name. " p
    ...
    // get vendor to check user_id
    function checkVendorOwner(){
